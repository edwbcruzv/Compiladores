from Lib_Compiladores.Lib_AFN_e import *
...
class CreadorAFNe:
    ...
    def CrearAutomataAFN(self,nombre_AFN,num_estados,str_lenguaje,str_estados_aceptacion,str_transiciones):
        
        #se valida el nombre
        if not(str==type(nombre_AFN) and len(nombre_AFN)>4):
            return "Error en el nombre o nombre muy corto"
        ...
        if not(str==type(str_lenguaje)):
            return "Error en el parametro de lenguaje"
        if not(str==type(str_estados_aceptacion)):
            return "Error en el parametro de estados de aceptacion"
        if not(str==type(str_transiciones)):
            return "Error en el parametro de transiciones"
        ...
        # S:"nombre estado"
        S_str="0"   #por determinado va a ser 0
